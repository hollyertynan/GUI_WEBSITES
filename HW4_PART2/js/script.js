...
function getValues() {
    ...
    for (i = firstMultiplicand; i <= lastMultiplicand + 1; i++) { // row
        ...
        for (j = firstMultiplier; j <= lastMultiplier + 1; j++) { // col
            if (i == firstMultiplicand && j == firstMultiplier) {
                ...
                multi_table += "<th class=\"table-light position-absolute\" style=\"width: 45px\">" + col_counter + "</th>";
                col_counter++;
    ...
    */
    ...
    return "<div class=\"overflow-auto\"><table class=\"table table-bordered table-dark\"></div>" + multi_table + "</table></div>";
}
...
$(document).ready(function() {
    ...
    (function($) {
        ...
        $("#add").click(function() {
            // Tab doesn't have add or remove anymore
            // https://stackoverflow.com/questions/16701277/no-such-method-add-for-tabs-widget-instance
            $("#tabs > ul").append('<li><a href="#newTab' + tabCount + '">' + tabCount + '</a><span class="ui-icon ui-icon-close" role="presentation">Remove Tab</span></li>');

            $("#tabs").append('<div id="newTab' + tabCount + '">' + getValues() + '</div>');
            tabCount++;
            $("#tabs").tabs("refresh");
        });
    ...
    (function($){
        var slider = {
            ...
            slide: function(event, ui) {
                var minMaxNum = $("#sliderMultiplier").slider("values");
                $("#first").val(minMaxNum[0]);
                $("#second").val(minMaxNum[1]);
                getValues();
            },
            create: function() {
                var minMaxNum = $("#sliderMultiplier").slider("values");
        ...
        // http://www.tutorialspark.com/jqueryUI/jQuery_UI_Slider_Events.php
        // https://api.jquerymobile.com/slider/
        $("#first").change(function() {
            $("#sliderMultiplier").slider("values", 0, $(this).val());
        });

        $("#second").change(function() {
            $("#sliderMultiplier").slider("values", 1, $(this).val());
        });
        
        $("#sliderMultiplier").slider(slider);
    })(jQuery);
    ...
    (function($){
        var slider = {
            ...
            range: true,
            slide: function() {
                var minMaxNum = $("#sliderMultiplicand").slider("values");
                $("#third").val(minMaxNum[0]);
                $("#fourth").val(minMaxNum[1]);
                getValues();
            },
            create: function() {
                var minMaxNum = $("#sliderMultiplicand").slider("values");
        ...
        $("#third").change(function() {
            $("#sliderMultiplicand").slider("values", 0, $(this).val());
        });

        $("#fourth").change(function() {
            $("#sliderMultiplicand").slider("values", 1, $(this).val());
        });

        $("#sliderMultiplicand").slider(slider);
    })(jQuery);
